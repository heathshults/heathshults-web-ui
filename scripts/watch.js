
const path = require('path');
const { watch } = require('gulp')
const renderAssets = require('./render-assets.js');
const renderJS = require('./render-scripts')
const renderEJS = require('./render-ejs');
const renderPug = require('./render-pug');
const renderSCSS = require('./render-scss');

// var destPath =  path.resolve(__dirname, '../www')
var srcPath = path.resolve(__dirname, '../src')

function watchers(cb) {
  cb = () => {if (typeof cb === 'function'){cb(null); called = true}}
  watch(`${srcPath}/*.ejs`, renderEJS.ejsit()), cb
  watch(`${srcPath}/pug/*.pug`, renderPug.renderPug()), cb
  watch([`${srcPath}/assets/img/**/*.{jpg,png,gif,svg}`, `${srcPath}/assets/content/**/*.{jpg,png,gif,svg}`], renderAssets.copy_images()), cb
  watch([`${srcPath}/scss/**/*.scss`], renderSCSS.renderSCSS()), cb
  watch([`${srcPath}/assets/**/*.css`], renderAssets.copy_css()), cb
  watch([`${srcPath}/js/*.{js,json,mjs,cjs,tsx}`, `!${srcPath}/js/HeathScript.js`], renderAssets.copy_js()), cb
  watch([`${srcPath}/js/HeathScript.js`], renderJS.HeathenScriptJS('../www/assets/js')), cb
}
exports.watchers = watchers
watchers()
