...
export class FramekitHighlightCode {
  ...
  async fetchCode() {
    ...
    try {
      ...
      const container: HTMLElement = this.el.shadowRoot.querySelector('div.hs-highlight-code-container');
  ...
  private parseCode(code: string): Promise<void> {
    return new Promise<void>(async (resolve, reject) => {
      ...
      if (container) {
        try {
          const highlightedCode: string = Prism.highlight(code, (this.language as any), (Prism.languages[this.language] as any));
      ...
      if (elements) {
        ...
        if (anchors) {
          anchors.forEach((anchor: HTMLElement) => {
            anchor.classList.add('hs-highlight-code-anchor');

            if (this.hideAnchor) {
              anchor.classList.add('hs-highlight-code-anchor-hidden');
            }
          });
        }
      }
  ...
  findNextAnchor(enter: boolean): Promise<FramekitHighlightCodeAnchor> {
    return new Promise<FramekitHighlightCodeAnchor>(async (resolve) => {
      const elements: NodeListOf<HTMLElement> = this.el.shadowRoot.querySelectorAll('span.hs-highlight-code-anchor');
  ...
  zoomCode(zoom: boolean): Promise<void> {
    return new Promise<void>((resolve) => {
      ...
      if (container) {
        container.style.setProperty('--hs-highlight-code-zoom', zoom ? '1.3' : '1');
      }
  ...
  render() {
    return <div class="hs-highlight-code-container">
      <code></code>
      <slot name="code"></slot>
    </div>;
  }
