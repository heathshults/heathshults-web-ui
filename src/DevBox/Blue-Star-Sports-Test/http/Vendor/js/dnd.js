...
(function ($) {
	...
	Draganddrop.prototype.dropHandler = function () {
		...
		_this.$el.on('drop', function (e) {
			if (_this.dragSrcEl !== this) {
				var dragSrcClone = $(_this.dragSrcEl).clone(true).removeClass('drag over'),
					dragDestClone = $(this).clone(true).removeClass('drag over'),
					$dragSrcEl = $(_this.dragSrcEl),
					$this = $(this);
	...
	$.fn.draganddrop = function () {
		...
		if (!data) this.data('draganddrop', new Draganddrop(this));
